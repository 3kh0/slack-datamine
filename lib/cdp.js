...
async function connect({ port = '9222', urlFilter = 'app.slack.com' } = {}) {
  ...
  const allowBlank = urlFilter === 'app.slack.com' !== '0';
  ...
  const navigate = async (url) => { await send('Page.enable'); await send('Page.navigate', { url }); };
  ...
  return { ws, target, send, evaluate, navigate, waitReady: (d) => waitReady(evaluate, d), close: () => ws.close() };
}
